        ...
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-form ref="spuBaseForm" :model="spu" label-width="120px" :rules="spuBaseInfoRules">
           ...
           <el-form-item label="商品介绍" prop="decript">
              <image-upload v-model="spu.decript" returnType="array"/>
            </el-form-item>

           <el-form-item label="商品图集" prop="images">
              <image-upload v-model="spu.images" returnType="array"/>
            </el-form-item>
            <el-form-item>
              ...
              <template slot-scope="scope">
                <el-row>
                  <el-col :span="24">
                    ...
                    <image-upload v-model="uploadImages" returnType="array"/>
                  </el-col>
                  <el-col :span="24">
                  ...
                  <el-col :span="24">
                    <el-card
                      ...
                      :key="index"
                    >
                      <image-preview :src="img" style="width:160px;height:120px"  alt=""/>
...
import BrandSelect from "@/views/commoin/brand-select.vue";
import CategoryCascader from "@/views/commoin/category-cascader.vue";
import PubSub from "pubsub-js";
import {addSpuInfo, getAttrGroupWithoutAttrs} from "@/api/product/productSpuadd";
import {listMemberLevelPage} from "@/api/member/memberLevel";
import {listSaleAttrAll} from "@/api/product/saleAttr";
...
export default {
  ...
  methods: {
    ...
    getMemberLevels() {
      ...
      listMemberLevelPage(queryParams).then(({ data }) => {
          this.dataResp.memberLevels = data.rows;
        })
        .catch(e => {
          console.log(e);
        });
    ...
    getShowSaleAttr() {
      ...
      if (!this.dataResp.steped[1]) {
        listSaleAttrAll(this.spu.catalogId).then(({ data }) => {
    ...
    showBaseAttrs() {
      if (!this.dataResp.steped[0]) {
         getAttrGroupWithoutAttrs(this.spu.catalogId).then(({ data }) => {
          //先对表单的baseAttrs进行初始化
          data.data.forEach(item => {
            let attrArray = [];
