  ...
  <div class="app-container">
    <el-form :model="queryParams.data" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
                  ...
                  <el-form-item label="满几件" prop="fullCount">
                    <el-input
                        v-model="queryParams.data.fullCount"
                        placeholder="请输入满几件"
                        clearable
                        @keyup.enter.native="handleQuery"
                    />
                  ...
                  <el-form-item label="打几折" prop="discount">
                    <el-input
                        v-model="queryParams.data.discount"
                        placeholder="请输入打几折"
                        clearable
                        @keyup.enter.native="handleQuery"
                    />
                  ...
                  <el-form-item label="是否叠加其他优惠[0-不可叠加，1-可叠加]" prop="addOther">
                    <el-select v-model="queryParams.data.addOther" placeholder="请选择是否叠加其他优惠[0-不可叠加，1-可叠加]" clearable>
                      <el-option
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
                        ...
                        <el-form-item label="是否叠加其他优惠[0-不可叠加，1-可叠加]" prop="addOther">
                          <el-radio-group v-model="form.addOther">
                            <el-radio
                                v-for="dict in dict.type.add_other"
                                :key="dict.value"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.loading = true;
        listSkuLadderPage(this.queryParams).then(response => {
          this.skuLadderList = response.data.rows;
