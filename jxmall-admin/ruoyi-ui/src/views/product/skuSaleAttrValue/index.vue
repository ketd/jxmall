...
<script>
  import {
      listSkuSaleAttrValuePage,
    getSkuSaleAttrValue,
    delSkuSaleAttrValue,
    addSkuSaleAttrValue,
    updateSkuSaleAttrValue ,
    exportSkuSaleAttrValue
  } from "@/api/product/skuSaleAttrValue";
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.loading = true;
        listSkuSaleAttrValuePage(this.queryParams).then(response => {
          this.skuSaleAttrValueList = response.data.rows;
  ...
  handleExport(row)
  {
    ...
    return exportSkuSaleAttrValue(ids);
    }).then(() => {
