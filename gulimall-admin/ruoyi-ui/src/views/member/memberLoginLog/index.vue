  ...
  <div class="app-container">
    <el-form :model="queryParams.data" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
                  ...
                  <el-form-item label="登录类型[1-web，2-app]" prop="loginType">
                    <el-select v-model="queryParams.data.loginType" placeholder="请选择登录类型[1-web，2-app]" clearable>
                      <el-option
                          v-for="dict in dict.type.login_type"
                          :key="dict.value"
                          :label="dict.label"
                          :value="dict.value"
                      />
      ...
      <el-table-column type="selection" width="55" align="center" />
              ...
              <el-table-column label="登录类型[1-web，2-app]" align="center" prop="loginType">
                <template slot-scope="scope">
                      <dict-tag :options="dict.type.login_type" :value="scope.row.loginType"/>
                </template>
              </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
                        ...
                        <el-form-item label="登录类型[1-web，2-app]" prop="loginType">
                          <el-select v-model="form.loginType" placeholder="请选择登录类型[1-web，2-app]">
                            <el-option
                                v-for="dict in dict.type.login_type"
                                :key="dict.value"
                                :label="dict.label"
...
<script>
  import {
      listMemberLoginLogPage,
    getMemberLoginLog,
    delMemberLoginLog,
    addMemberLoginLog,
    updateMemberLoginLog ,
    exportMemberLoginLog
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.loading = true;
        listMemberLoginLogPage(this.queryParams).then(response => {
          this.memberLoginLogList = response.data.rows;
  ...
  handleExport(row)
  {
    ...
    return exportMemberLoginLog(ids);
    }).then(() => {
