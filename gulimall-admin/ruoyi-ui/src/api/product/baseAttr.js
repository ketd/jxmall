...
// 分页查询商品属性列表
export function listBaseAttrPage(queryParams) {
  queryParams.data.attrType=1;
  return axios({
    baseURL: baseURL,
    url: '/product/Attr/base/list/page',
    method: 'post',
    data: queryParams
  })
}

export function findAttrGroupRelation(queryParams) {
  return axios({
    baseURL: baseURL,
    url: '/product/Attr/base/list/findAllAttrByAttrGroupId/page',
    method: 'post',
    data: queryParams
  })
}
...
export function linkAttr(attrId, attrGroupIds) {
  return axios({
    baseURL: baseURL,
    url: `/product/Attr/link?attrId=${attrId}`,
    method: 'POST',
    data: attrGroupIds
  });
}
