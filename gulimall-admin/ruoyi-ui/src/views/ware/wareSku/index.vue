  ...
  <div class="app-container">
    <el-form :model="queryParams.data" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
                  ...
                  <el-form-item label="库存数" prop="stock">
                    <el-input
                        v-model="queryParams.data.stock"
                        placeholder="请输入库存数"
                        clearable
                        @keyup.enter.native="handleQuery"
                    />
                  ...
                  <el-form-item label="锁定库存" prop="stockLocked">
                    <el-input
                        v-model="queryParams.data.stockLocked"
                        placeholder="请输入锁定库存"
                        clearable
                        @keyup.enter.native="handleQuery"
                    />
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.loading = true;
        listWareSkuPage(this.queryParams).then(response => {
          this.wareSkuList = response.data.rows;
