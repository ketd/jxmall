package com.ketd.product.utils;
...
//            throw new RuntimeException("递减因子必须大于0");
    ...
    public List<String> findAlllrange(String key) {
    ...
    public List<String> findValuesByPattern(String pattern) {
        // Get all keys matching the provided pattern
    ...
    public void updateChatMessage(int index, String key, String value) {
        redisTemplate.opsForList().set(key, index, value);
    }

    public void deleteChatMessage(String key, int index) {
        // 获取列表长度
        ...
        if (index >= 0 && index < listSize) {
            // 删除列表中指定索引的元素
            redisTemplate.opsForList().remove(key, index, Objects.requireNonNull(redisTemplate.opsForList().index(key, index)));
        } else {
            // 处理索引超出范围的情况
     ...
     * @param key 键名
     * @param <T> 返回对象的类型
     * @return 从Redis获取的对象，如果键不存在或转换失败，返回null
     */
